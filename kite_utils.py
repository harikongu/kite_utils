...
class KiteUtil(object):
    ...
    @staticmethod
    def get_profit(buy_price, sell_price, qty, exchange, trade_type):
        # price_diff = (sell_price - buy_price) * QTY
        total_buy_price = buy_price * qty
        total_sell_price = sell_price * qty

        if exchange == KiteUtil.EXCHANGE_BSE:
            transaction_charges = 3
        elif exchange == KiteUtil.EXCHANGE_NSE:
            buy_transaction = total_buy_price * KiteUtil.TRANSACTION_CHARGES_NSE
            sell_transaction = total_sell_price * KiteUtil.TRANSACTION_CHARGES_NSE
            transaction_charges = buy_transaction + sell_transaction

        if trade_type == KiteUtil.TRADE_TYPE_INTRADAY:
            buy_brokerage = total_buy_price * KiteUtil.BROKERAGE_INTRADAY
            sell_brokerage = total_sell_price * KiteUtil.BROKERAGE_INTRADAY

            buy_brokerage = buy_brokerage if buy_brokerage < KiteUtil.BROKERAGE_INTRADAY_FLAT else KiteUtil.BROKERAGE_INTRADAY_FLAT
            sell_brokerage = sell_brokerage if sell_brokerage < KiteUtil.BROKERAGE_INTRADAY_FLAT else KiteUtil.BROKERAGE_INTRADAY_FLAT

            brokerage = buy_brokerage + sell_brokerage
            stt_ctt = sell_price * KiteUtil.STT_CTT_INTRADAY
        elif trade_type == KiteUtil.TRADE_TYPE_DELIVERY:
            stt_ctt = (sell_price * KiteUtil.STT_CTT_DELIVERY) + (buy_price * KiteUtil.STT_CTT_DELIVERY)
            brokerage = 0

        gst = (brokerage + transaction_charges) * KiteUtil.GST
        sebi_buy_charges = buy_price * qty * KiteUtil.SEBI_CHARGES
        sebi_sell_charges = sell_price * qty * KiteUtil.SEBI_CHARGES
    ...
    def is_tradingday():
        ...
        if weekday_index == 5 or weekday_index == 6:
            ...
            if today in [datetime.strptime(d, "%d/%m/%Y").date() for d in KiteUtil.holiday_list]:
                return False
            else:
                return True

    @staticmethod
    def is_indian_timezone():
        if tzname[1] == 'IST':
            return True
        else:
            return False
    ...
    def is_pre_market_hour():
        ...
        pre_market_open_time = time(*map(int, KiteUtil.PRE_MARKET_OPEN_TIME.split(':')))
        pre_market_close_time = time(*map(int, KiteUtil.PHYSICAL_MARKET_OPEN_TIME.split(':')))
    ...
    def is_market_hour():
        ...
        physical_market_open_time = time(*map(int, KiteUtil.PHYSICAL_MARKET_OPEN_TIME.split(':')))
        physical_market_close_time = time(*map(int, KiteUtil.PHYSICAL_MARKET_CLOSE_TIME.split(':')))
